...
int cycle=0;
map<NodeID, int> time_set;
map<NodeID, vector<int>> time_checked;
...
bool BFSVerifier(const Graph &g, NodeID source,
  ...
  for (auto it = to_visit.begin(); it != to_visit.end(); it++) {
    ...
    //ahmed 
    // Add this node to time map if not there.
    time_set[u] = cycle;    
    ...
    for (NodeID v : g.out_neigh(u)) {
      ...
      // If visited.
      // ahmed
      // set the time it's checked for visited.
      // do: printf("ahmed: Adding to time_checked\n");
      auto itr = time_checked.find(v);
      if( itr != time_checked.end())
      {
        vector<int> &vec = itr->second;
        vec.push_back(cycle);
      }
      // vector<int> &vec = time_checked[v];
      // vec.push_back(cycle);
...
int main(int argc, char* argv[]) {
  ...
  {
    // do: printf("Node ID: %d\n", u);
    // do: printf("Time_set: %d\n", time_set[u]);

    // do: printf("Time Accessed: ");

    // do: printf("\n");
  }

  // do: printf("=========\n");
  for(const auto &node: time_checked)
  {
